...
/**
 * 本类的目的是演示多个线程并发执行时的问题。 有两个生产者以及两个消费者，我们的目的是生产一个消费一个。 所以通过flag标志位确定生产与消费交替实现。
 * <p>
 * 如果生产者或消费者中的while 和 notifyAll 换成 if 和 notify 会出现如下现象。 如果用if判断flag标志位，那么在同步函数内，t1开始执行获得资源判断flag为假，跳过wait，执行后面语句。并在时间片用光之前又执行到了if，此时flag为真
...
public class B5_NotifyAll {
    ...
    private static class Resource {
        ...
        /**
         * 生产方法，根据标志位判断商品是否被消费掉了。 若没有被消费则等待
         */
        ...
        /**
         * 消费者方法，根据标志位判断是否有商品可以被消费 若没有可消费的商品则等待
         */
