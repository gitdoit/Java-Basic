...
/**
 * yield()方法是交出自己的执行权限，使自己从执行态强制转换为就绪态，即重新抢夺资源。 好不容易拿到手的资源被yield给夺走了，且让出的执行权只能被同等或高等优先级的线程抢夺。(但当此时没有满足此条件的线程时，则低优先级的线程也会得到执行权)
 * 这更想是使被调用线程暂停一下 注意，它与sleep方法不同，sleep方法是使该线程强制进入有限等待状态，经过指定时间后才转换为就绪态。 而yield方法，使该线程从执行态转为就绪，而不阻塞。
