...
/**
 * 两个线程并发的对count进行--操作并打印。如果线程A在执行到打印语句的时候时间片用完，它会保存现场信息 如果此时count=5，由于还没有执行打印语句，所以没有执行--操作。所以共享信息count没有发生改变
 * 线程B获得执行资源，此时线程B中的count还是为5.打印“5”并使count=4；休眠，线程A获得执行资源 执行打印语句打印“5”，并使count=count - 1；即count = 4；则会出现连续打印两个相同数的情况。
 * 即两个线程没有同步，如果在判断while循环时count = 0，可以执行。但是判断完之后时间片用完 线程B进来，打印并修改了count值，休眠。线程A执行，在打印语句中重新读取count值此时count被B修改为-1，则会打印-1
 * <p>
