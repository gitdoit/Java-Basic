...
/**
 * @author liujianxin
 * @date 2018-07-09 15:43 描述信息：Terminal是一个消耗流操作
 **/
public class TerminalStreamTest {
     ...
     * <p>
     * reduce的作用是将stream元素组合起来 这样说好像有些不对劲，这个操作会将stream中所有的元素都参与到计算中 然后返回一个结果。 你可以将这些元素组合起来然后返回一个结果，也可以将这些元素求一下平均值，最大值，等等
     * <p>
     * <p>
     * 一个参数的Reduce操作，操作过程是 传入参数(a[0],a[1]) 自定义操作返回结果 c(返回值类型要和参数类型一样) 这个返回值会和下一个元素重复上述操作
     */
    @Test
    ...
    /**
     * 三个参数的Reduce 跟前两个用法有很大区别 第一个参数R，就是返回值类型，它没有象前两个操作那样对返回值做了限定 这个U可以是任何类型，所以这样用法就多了起来，我们可以对一个字符数组进行求所有字符串长度的和，然后返回这个长度值
     * <p>
    ...
    public void testReduce3() {
        ...
        Stream<Integer> stream2 = Stream.of(1, 2, 3, 4);
        BigDecimal reduce3 = stream2.parallel()
                .reduce(new BigDecimal(0), (r, t) -> r.add(new BigDecimal(t)), BigDecimal::multiply);
