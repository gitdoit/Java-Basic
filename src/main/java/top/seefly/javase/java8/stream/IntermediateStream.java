...
/**
 * @author liujianxin
 * @date 2018-07-04 23:12 描述信息：Intermediate类型的流操作可以在使用方法之后再返回一个流 对这个流进而进行其他后继操作
 **/
public class IntermediateStream {
     ...
     * flatMap直译过来就是 扁平化映射，就是合并多个流成为一个流 他需要传的Function中限定返回值只能时一个流，而map中的Function没有做限定
     * 既然map和flatMap的参数都是Function，区别只是一个限定返回值一个没有 那么能不能将map代替flatMap呢，答案是不行的，因为即使给map传一个返回值是stream的Function
    ...
    /**
     * 测试peek操作，直译过来就是瞥一眼，API上介绍说，这个通常用来debug用的 这个操作跟forEach很象，但是forEach是一个消耗流操作，这个不是 他就是瞥一眼，不能对元素有任何操作.
     * 注意：如果一个流，在没有被消耗掉之前，使用peek他是不会起作用的。 也就是说，你想使用peek瞥一眼，只有在流被消耗掉的时候它才去瞥一眼，不然不会
     */
    @Test
    ...
    /**
     * findFirst和findAny findFist是找到第一个 findAny是找到任意一个 示例：https://zhidao.baidu.com/question/1498838916214323339.html
