package top.seefly.javase.nio.selector;
...
public class SelectorDemo {
    ...
    /**
     * 将一个Channel注册到Selector上 并告诉Selector你对这个通道上的那些事件感兴趣，需要注意的是这个Channel需要是非阻塞的。 这样一来，就不必单开一个线程以阻塞的方式等待你感兴趣的事件到来。
     */
    ...
    /**
     * 上面注册之后的返回值：SelectionKey，它包含了四个信息 interest集合：即我们注册时，传入的感兴趣的事件集合 ready集合 Channel Selector
     */
    ...
    /**
     * selector.select() 有三个重载方法，这些方法返回你所感兴趣的事件已经就绪的通道 如连接、接受、读、写已经就绪的通道。 这三个方法的返回值int，表示已经有多少个通道就绪。
     * 亦即，自上次调用select()方法后有多少通道变成就绪状态。
     * <p>
     * selector.selectedKeys() 一旦select()方法表明有一个或多个通道就绪之后，可以调用此方法。 问题：返回值是这个Selector中所有注册的Channel还是准备好的那些？？ 是已经准备好的那些
     */
    ...
    /**
     * 由于selectorA.select()方法会阻塞线程，所以需要有一种方式 将被阻塞的线程唤醒，这个时候可以在其它线程中调用selectorA.wakeUp()。 注意，两个线程操作的Selector要是同一个。
     * <p>
     * 另外，如果没有线程由于调用select方法被阻塞，那么此时调用唤醒方法 那么下一个调用select方法的线程会立即返回。
     */
