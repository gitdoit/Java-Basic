package top.seefly.javase.nio.file;
...
/**
 * TODO 完善 https://www.cnblogs.com/digdeep/p/4478734.html
 * <p>
...
public class FilesDemo {
    ...
    /**
     * 通过Files工具类读取文本文件中所有的行 Files.readAllLines底层循环调用BufferedReader.readLine，把所有行放到List中 如果文本文件大小未知，不建议使用
     */
    @Test
    public void testReadAllLines() throws IOException {
        ...
        Files.write(Paths.get("E:\\test\\io12.txt"), "haha中".getBytes(StandardCharsets.UTF_8),
                StandardOpenOption.CREATE);
    }
    
    /**
     * 通过Files获取BufferedReader bufferedReader.lines()方法返回一个流，若使用变量接受，则记得关闭
     */
    @Test
    ...
    /**
     * 通过调用Files.lines底层调用底层调用BufferedReader.lines()方法。 这个方法返回一个流，不像Files.readAllLines一样一次性把所有数据加载到内存中
    ...
    /**
     * 万一能用到呢？ 可以用来迭代删除一个未知深度的文件夹，window上一个delete的事情
     * <p>
     * walk方法说它将给定路径作为根，进行深度优先遍历 跳过被安全管理器保护的文件，默认情况下也不会遍历文件链接。 也是弱引用的(可能是在遍历生成流的过程中某个文件被更新了，这里是察觉不到的)
     * 可以指定最大深度，若为Integer.MAX，则无限深度。 流中元素为Path
     * <p>
