...
/**
 * 在使用继承时，数据域也会出现覆盖现象。但是在实例化时也会为其分配存储空间 类的数据域不具有多态性。 多态时，引用变量只能调用其声明类型所拥有的方法，但如果实际类型也拥有这个方法，那么此时会出现多态性，即调用实际类型中的方法，而同时
 * 因为数据域没有多态性，所以实际调用的那个方法会使用同类中的数据域。
 * <p>
 * <p>
 * 用来学习继承 子类中有俩方法，一个公共的shou方法：返回一段字符串，一个私有的show方法：返回一段字符串 父类中有四个方法，公共的shou方法：返回一段字符串，私有的show:返回一段字符串，公共的print：打印show（）
 * 公共的printDame:打印shou()
 * <p>
 * 父类  实例 = new 子类(); 如果要使用多态那么必须在父类中定义一个方法，并在子类中覆盖/重写.即子类和父类中都有 ‘方法’这个方法 这样就可以用父类变量引用子类中的覆盖方法 ：实例.方法
 * （方法在子类中被重写了才能引用到子类中的这个方法） 如果父类中没有这个方法而子类中有，那么需要进行显示转换才能使用子类中的方法 注：转换不会创建一个新的对象 (子类)(实例).方法1
 * <p>
 * 表达式(实例1 instanceof 实例2) 得到的是一个boolean值，类型相同为真 表达式(实例1 == 实例2) 的意思是判断两个变量是否指向同一个实例，即地址是否相同 表达式：实例.equals(实例1)
 * 的意思是判断两个变量是否指向同一个实例，即地址是否相同 表达式：字符串.equals(字符串1) 判断两个字符串变量的地址是否相同 表达式：字符串 == 字符串1 判断两个字符串的内容是否一样，不考虑地址
 */
