package top.seefly.javase.jvm.loadclass;
 ...
 * <p>
 * 三种类加载器 1、启动类加载器： BootstrapClassLoader 负责加载 ${JAVA_HOME}/jre/lib 文件夹下的类库 2、扩展类加载器： ExtensionClassLoader 负责加载
 * ${JAVA_HOME}/jre/lib/ext 文件夹下的类库 3、应用程序类加载器： ApplicationClassLoader 该类加载器由 sun.misc.Launcher$AppClassLoader来实现
 * 它负责加载用户类路径（ClassPath）所指定的类，开发者可以直接使用该类加载器 如果应用程序中没有自定义过自己的类加载器，一般情况下这个就是程序中默认的类加载器。
 * <p>
 * <p>
 * 双亲委派机制。 听着很高大上，其实很简单。比如A类的加载器是AppClassLoader(其实我们自己写的类的加载器都是AppClassLoader)， AppClassLoader不会自己去加载类，而会委ExtClassLoader进行加载，那么到了ExtClassLoader类加载器的时候，
 * 它也不会自己去加载，而是委托BootStrap类加载器进行加载，就这样一层一层往上委托，如果Bootstrap类加载器无法进行加载的话，再一层层往下走。
 * <p>
 * 总的来说，在加载类的时候先不要自己加载，调用父加载器去加载，父类也是同样的逻辑，直到父类为空的时候 那时候就直接调用BootStrapClassLoader了，如果没有加载到，儿子再加载，孙子在加载...
 * 这种模式的好处就是保证了加载到的类是安全的，不是被篡改过的，像是lang包下的都是由根加载器去加载的，避免了 随便一个类加载器都能替换String
 *
 * @author liujianxin
        ...
        ClassLoader classLoader = new ClassLoader() {
            ...
            public Class<?> loadClass(String name) throws ClassNotFoundException {
                ...
                InputStream resourceAsStream = getClass()
                        .getResourceAsStream(name.substring(name.lastIndexOf(".") + 1) + ".class");
