...
/**
 * Arraylist 与 LinkedList 区别? 1. 是否保证线程安全： ArrayList 和 LinkedList 都是不同步的，也就是不保证线程安全；
 * <p>
 * 2. 底层数据结构： Arraylist 底层使用的是 Object 数组；LinkedList 底层使用的是 双向链表 数据结构
 * <p>
 * 3. 插入和删除是否受元素位置的影响： 1、ArrayList 采用数组存储，所以插入和删除元素的时间复杂度受元素位置的影响。 比如：执行add(E e) 方法的时候， ArrayList
...
public class ListDemo {
    ...
    /**
     * Arrays.copyOf()底层调用的也是System.arraycopy 这个方法的目的就是为了方便的给数组扩容
     */
    @Test
    ...
    /**
     * list.toArray方法很有意思 如果传入的参数数组容量足够大，那么将原列表中的元素复制一份到这个参数数组中 如果不够大，那么就根据参数列表类型创建一个新的数组，当作返回值返回回去。
     * <p>
    ...
    /**
     * arrayList.ensureCapacity(n); 这方法是给用户用的，目的就是让用户在添加大量数据前 能够手动扩容列表容量 避免列表自己一次次的扩容，提升效率。
     */
    @Test
